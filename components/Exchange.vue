  ...
  <v-container class="mt-5">
    ...
    >
      ...
      <v-form v-if="!sell" ref="form" v-model="valid" lazy-validation>
        ...
        <div class="box-exc">
          ...
          <br />
          <p v-if="crypto_id">
            курс обменна: 1 {{ crypto_id }} = {{ cyrs_crypto }} rub
          </p>
        </div>
        <v-container fluid>
          <v-row align="center">
    ...
    >
      <div v-if="allert" class="box-alert">
        <v-alert style="text-align: initial" class="alertcucces" type="success">
      ...
      <v-form ref="formdata" v-model="valid" lazy-validation>
        ...
        <div class="mt-5" v-if="text_operacion === 'Покупка'">
          <p>
            Отправте {{ result_rub }} рублей по следующим реквизитам:
            <br />
        ...
        <v-text-field
          ...
          required
        ></v-text-field>
        <span v-if="text_operacion === 'Продажа'"
          >Вы продаете {{ result_crypto }} {{ crypto_id }} за
          {{ result_rub }} рублей</span
        >
        <span v-if="text_operacion === 'Покупка'"
          >Вы покупаете {{ result_crypto }} {{ crypto_id }} за
          {{ result_rub }} рублей</span
        >
        <v-checkbox
          v-model="checkbox"
...
export default {
  ...
  data() {
    return {
      valid: true,
      ethereumlRules: [(v) => !!v || "Адрес обязателен"],
      fiolRules: [(v) => !!v || "ФИО обязательно"],
      bitcoinlRules: [(v) => !!v || "Адрес обязателен"],
      emailRules: [
        (v) => !!v || "E-mail обязателен",
        (v) => /.+@.+\..+/.test(v) || "E-mail неверно введен",
      ],
      cardlRules: [(v) => !!v || "Данные карты обязательны"],
      datapay: false,
      sell: false,
      allert: false,
      result_crypto: 0,
